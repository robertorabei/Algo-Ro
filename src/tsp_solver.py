...
def main():
    ...
    if f == 0:
        ...
        obj, tour, solve_time, _, _ , _ = solve_mtz(distances, False)
    elif f == 1:
        method_name = "MTZ (Relaxé)"
        obj, tour, solve_time, _, _ , _ = solve_mtz(distances, True)
    elif f == 2:
        method_name = "DFJ Enumératif (Entier)"
        obj, tour, solve_time, _, _ , _ = solve_dfj_enum(distances, False)
    elif f == 3:
        method_name = "DFJ Enumératif (Relaxé)"
        obj, tour, solve_time, _, _ , _ = solve_dfj_enum(distances, True)
    elif f == 4:
