...
REGEX_N = r"instance_(\d+)_"

def benchmark():
    with open(OUTPUT_CSV, mode='w', newline='') as f:
        writer = csv.writer(f)
        header = ["instance", "formulation", "obj_int", "time_int", "obj_relax", "time_relax", "gap", "vars", "constr", "iterations"]
        writer.writerow(header)

        files = [f for f in os.listdir(INSTANCES_DIR)]
        files.sort(key=lambda x: (int(re.search(REGEX_N, x).group(1)), x))

        for filename in files:
            filepath =  os.path.join(INSTANCES_DIR, filename)
            match = re.search(REGEX_N, filename)
            if not match: continue
            
            n, distances = extraire(filepath)
            print(f"--- Traitement de {filename} (n={n}) ---")
            ...
            writer.writerow([instance_name, "DFJ_iter", round(obj_iter, 2), round(time_iter, 4), 
                             "-", "-", "-", vars_iter, constr_iter, iter_count])
